...
public class Database{
	...
	public ArrayList<Statistics> addData(User current) throws IOException {
		...
		int sheetNum = in.nextInt();
		workbook = new XSSFWorkbook(file1+".xlsx");
		*/
	...
	public void uploadData() throws IOException {
		...
		Reader in = new FileReader("Input Format.csv");
		Iterable<CSVRecord> records = CSVFormat.RFC4180.parse(in);
		for (CSVRecord record : records) {
		    double time = Double.parseDouble(record.get(0));
		    double distance = Double.parseDouble(record.get(1));
		    double altitude = Double.parseDouble(record.get(2));
		    String date = record.get(3);
		    Statistics stat = new Statistics(time,distance,altitude,date);
		    stats.add(stat);
		}
